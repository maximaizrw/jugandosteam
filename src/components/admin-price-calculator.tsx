...
import { ArrowRight, Loader2, AlertCircle, TrendingUp, Save } from "lucide-react";
...
export function AdminPriceCalculator() {
  ...
  const { exchangeRates, isLoading: isLoadingRate, error: rateError, setCustomRate } = useExchangeRate();
  const [customRateInput, setCustomRateInput] = useState("");
  const { toast } = useToast();

  useEffect(() => {
    if (exchangeRates) {
      setCustomRateInput(exchangeRates.custom.toString());
    }
  }, [exchangeRates]);
  ...
  const handleSaveCustomRate = () => {
    ...
    if (!isNaN(newRate) && newRate > 0) {
      ...
      toast({
        ...
        description: `El valor del Dólar Personalizado se actualizó a ${formatCurrency(newRate)}.`,
      });
    } else {
      toast({
        variant: "destructive",
        title: "Valor no válido",
        description: "Por favor, ingresá un número válido para la cotización.",
      });
    }
  };
  ...
  const handleCalculate = async (e: React.FormEvent) => {
    ...
    // Custom price calculation (10% profit)
    const baseArsCustom = usdPrice * exchangeRates.custom;
    const finalPriceCustom = Math.round((baseArsCustom * 1.10) / 5) * 5;
    const profitCustom = finalPriceCustom - baseArsCustom;
    ...
    setResult({
      ...
      costs: {
        ...
        card: baseArsCard,
        custom: baseArsCustom,
        eneba: baseArsEneba,
      },
      prices: { 
        crypto: finalPriceCrypto, 
        card: finalPriceCard,
        custom: finalPriceCustom,
  ...
  return (
    <div className="space-y-6">
      <Card className="w-full shadow-lg">
        <CardHeader>
          <CardTitle>Dólar Personalizado</CardTitle>
          <CardDescription>
            Establecé tu propio valor para el dólar. Se guardará en tu navegador.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col sm:flex-row items-center gap-3">
          <Input
            ...
            aria-label="Valor del dólar personalizado"
          />
          <Button onClick={handleSaveCustomRate} className="w-full sm:w-auto">
      ...
      <Card className="w-full shadow-lg overflow-hidden">
        <CardHeader>
          ...
          <CardDescription>
            Ingresá el costo en dólares para obtener los precios de venta.
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <div className="w-full space-y-4">
                ...
                <div className="p-4 border rounded-lg border-primary">
                    <div className="flex justify-between items-center w-full mb-3">
                        <div>
                            <h3 className="font-semibold text-lg text-primary">Cálculo con Dólar Personalizado</h3>
                            <p className="text-xs text-muted-foreground">Ganancia: 10%</p>
                        </div>
                        <div className="text-right p-2 border rounded-lg bg-secondary/50">
                            <div className="text-xs text-muted-foreground flex items-center gap-1 justify-end"><TrendingUp size={12}/> Tu Dólar</div>
                            <p className="font-semibold">{formatCurrency(result.exchangeRates.custom)}</p>
                        </div>
                    </div>
                    <Separator className="my-2" />
                    <div className="space-y-1 text-sm">
                        <div className="flex justify-between"><span className="text-muted-foreground">Costo base (USD a ARS)</span><span className="font-medium">{formatCurrency(result.costs.custom)}</span></div>
                        <div className="flex justify-between"><span className="text-muted-foreground">Tu ganancia (10%)</span><span className="font-medium text-green-400">+ {formatCurrency(result.profit.custom)}</span></div>
                    </div>
                    <Separator className="my-2" />
                    <div className="flex justify-between items-center mt-2">
                        <span className="text-lg font-bold">Precio Final de Venta</span>
                        <span className="text-2xl font-bold text-primary">{formatCurrency(result.prices.custom)}</span>
                    </div>
                </div>
                ...
                <div className="p-4 border rounded-lg">
                    <div className="flex justify-between items-center w-full mb-3">
                        <div>
                            <h3 className="font-semibold text-lg text-accent">Cálculo con Dólar Cripto</h3>
                             <p className="text-xs text-muted-foreground">Ganancia: 10%</p>
                        </div>
                        <div className="text-right p-2 border rounded-lg bg-secondary/50">
                    ...
                    <div className="flex justify-between items-center mt-2">
                        <span className="text-lg font-bold">Precio Final de Venta</span>
                        <span className="text-2xl font-bold text-accent">{formatCurrency(result.prices.crypto)}</span>
                    </div>
                </div>
                ...
                <div className="p-4 border rounded-lg">
                    <div className="flex justify-between items-center w-full mb-3">
                        <div>
                            <h3 className="font-semibold text-lg text-secondary-foreground/80">Cálculo con Dólar Tarjeta</h3>
                    ...
                    <div className="flex justify-between items-center mt-2">
                        <span className="text-lg font-bold">Precio Final de Venta</span>
                        <span className="text-2xl font-bold text-secondary-foreground/80">{formatCurrency(result.prices.card)}</span>
                    </div>
                </div>
